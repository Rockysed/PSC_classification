...
btd_app_difference = np.empty([0, 6216])
...
ind_red = [i for j in (range(107), range(137, 142)) for i in j]
for file in files:
  ...
  wave_min = mipas_file["wave_min"][ind_red]
  ...
  for i in range(0, radiance_l[:, 0].size-1):
    ...
    if htang_l[i] >=15 and htang_l[i] <=30:
      if ci_idx >= 0.5 and ci_idx <= 4.5:
        rad = radiance_l[i, ind_red].reshape(1, 112)
        ...
        btd_a_r_difference = btd_r_difference.reshape(1, 6216)
        btd_app_difference = np.append(btd_app_difference, btd_a_r_difference, 0)
        #BTDs Reinhold
        btd_r = BT((abs(rad_app[ct,110])*(1e-9)), 833) - BT(abs(rad_app[ct,111]*(1e-9)), 949)
...
#save to file  
 ...
 #save btds to pandas dataframe
  if ci_idx_app.shape[0] > 0:
   ...
   df_btd.to_hdf(os.path.join('../data/mipas_pd_red', date + '.h5'), key='df_btd', mode='w') 
   ...
   df_btd_scaled.to_hdf(os.path.join('../data/mipas_pd_red', date + '.h5'), key='df_btd_scaled')
   ...
   df_features.to_hdf(os.path.join('../data/mipas_pd_red', date + '.h5'), key='df_features',
          format='table')
   ...
   df_features_scaled.to_hdf(os.path.join('../data/mipas_pd_red', date + '.h5'), key='df_features_scaled',
          format='table')
   ...
   df_reduced.to_hdf(os.path.join('../data/mipas_pd_red', date + '.h5'), key='df_reduced',
          format='table')
